 ...
 * next transfer
 */
static void msm_spi_set_spi_config(struct msm_spi *dd, int bpw, int msgmode)
{
 ...
 */
static u32 msm_spi_set_spi_io_control(struct msm_spi *dd, struct spi_device *spi, int cs_change)
{
...
static void msm_spi_process_transfer(struct msm_spi *dd)
{
	...
	msm_spi_set_spi_config(dd, bpw, dd->cur_msg->spi->mode);
	msm_spi_set_qup_config(dd, bpw);
	spi_ioc = msm_spi_set_spi_io_control(dd, dd->cur_msg->spi, dd->cur_transfer->cs_change);
...
int spi_qsd_set_deassert_clocks(struct spi_device *spi, int clocks) {
	struct msm_spi *dd = spi_master_get_devdata(spi->master);

    writel_relaxed(clocks, dd->base + SPI_DEASSERT_WAIT);

    return 0;
}
EXPORT_SYMBOL_GPL(spi_qsd_set_deassert_clocks);
/* >>> QEEXO: Deassert wait time control */

/* <<< QEEXO: Cyclic SPI DMA transfers */
static int spi_qsd_queue_transfers(struct msm_spi *dd, dma_addr_t tx_dma, dma_addr_t rx_dma, int len) {
	u32 cons_flags, prod_flags;
	int ret;
	int queued = 0;

    /* msm_spi_bam_begin_transfer */
	cons_flags = SPS_IOVEC_FLAG_EOT | SPS_IOVEC_FLAG_NWD;
	prod_flags = tx_dma ? 0 : cons_flags;

    if (rx_dma) {
        ret = sps_transfer_one(dd->bam.prod.handle, rx_dma,
            len, dd, prod_flags);
        if (ret < 0) {
            dev_err(dd->dev, "%s: Failed to queue producer BAM transfer", __func__);
            return ret;
        }
        queued++;
    }
    if (tx_dma) {
        ret = sps_transfer_one(dd->bam.cons.handle, tx_dma,
            len, dd, cons_flags);
        if (ret < 0) {
            dev_err(dd->dev, "%s: Failed to queue consumer BAM transfer", __func__);
            return ret;
        }
        queued++;
    }
...
int spi_qsd_run_cyclic_dma(struct spi_device *spi, int bpw, int speed_hz, int auto_cs, dma_addr_t tx_dma, dma_addr_t rx_dma, int len) {
	struct msm_spi *dd = spi_master_get_devdata(spi->master);
	int ret = 0;
	u32 timeout;
	u32 read_count;
    ...
    /* catch improper use of cur_msg/cur_transfer */    
    dd->cur_msg = NULL;
    dd->cur_transfer = NULL;
    ...
    /* msm_spi_process_message */
    if (dd->qup_ver)
		write_force_cs(dd, 0);

    /* msm_spi_process_transfer */
    dd->cur_msg_len = len;
    dd->tx_bytes_remaining = len;
    dd->rx_bytes_remaining = len;

    /* catch improper use of read_buf/write_buf */
    dd->read_buf = rx_dma ? (void *)~0 : NULL;
    dd->write_buf = tx_dma ? (void *)~0 : NULL;
    dd->bytes_per_word = (bpw + 7) / 8;
    msm_spi_clock_set(dd, speed_hz);

	timeout = 100 * msecs_to_jiffies(
			DIV_ROUND_UP(len * 8,
			DIV_ROUND_UP(speed_hz, MSEC_PER_SEC)));

    read_count = DIV_ROUND_UP(len, dd->bytes_per_word);

	if (msm_spi_set_state(dd, SPI_OP_STATE_RESET)) {
		dev_err(dd->dev, "%s: Error setting QUP to reset-state", __func__);
		ret = -EIO;
		goto error;
	}

    /* Skipping the usual sanity checks for BAM suitability.
       You better not call this function if BAM is unsuitable,
    ...
    /* msm_spi_process_transfer */
    msm_spi_set_qup_io_modes(dd);
    msm_spi_set_spi_config(dd, bpw, spi->mode);
    msm_spi_set_qup_config(dd, bpw);
	spi_ioc = msm_spi_set_spi_io_control(dd, spi, auto_cs);
	msm_spi_set_qup_op_mask(dd);

    msm_spi_set_mx_counts(dd, 0);

    ret = msm_spi_set_state(dd, SPI_OP_STATE_RUN);
    if (ret < 0) {
        dev_err(dd->dev, "%s: Failed to set QUP state to run", __func__);
        goto error;
    }

    /* We init the completion only once. SPS BAM calls complete() on the completion,
    and the completion uses an internal counter to count (completions - wakeups).
    Therefore, we will effectively be woken up by each completed transfer exactly
    once. */
    init_completion(&dd->transfer_complete);

    ret = spi_qsd_queue_transfers(dd, tx_dma, rx_dma, len);
    if (ret < 0) {
        goto error;
    }

    while(dd->transfer_pending) {
        int queued = spi_qsd_queue_transfers(dd, tx_dma, rx_dma, len);
        if (queued < 0) {
            ret = queued;
            goto error;
        }

        /* Each queued pipe produces a completion. Therefore, we have to
        wait once for each active pipe direction. */
        while(queued-- > 0) {
            ret = wait_for_completion_timeout(&dd->transfer_complete, timeout);
            if (!ret) {
                dev_err(dd->dev, "%s: SPI transaction timeout", __func__);
                ret = -EIO;
                goto error;
            }
        }
...
EXPORT_SYMBOL_GPL(spi_qsd_run_cyclic_dma);

int spi_qsd_stop_cyclic_dma(struct spi_device *spi) {
	struct msm_spi *dd = spi_master_get_devdata(spi->master);

    /* wake up and stop run_cyclic_dma thread */
	dd->transfer_pending = 0;
    complete(&dd->transfer_complete);

    return 0;
}
EXPORT_SYMBOL_GPL(spi_qsd_stop_cyclic_dma);
/* >>> QEEXO: Cyclic SPI DMA transfers */
