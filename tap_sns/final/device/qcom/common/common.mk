...
INIT += init.pantech.usb.rc
INIT += init.pantech.usb.sh
INIT += init.pantech.preload.sh
# endif
...
KEYPAD += atmel_mxt_540s.kl
KEYPAD += stmicro_fts_ts.kl
...
##<!-- FEATURE_PANTECH_KEY_CERTIFICATION
ifeq ($(TARGET_BUILD_PRJ_NAME),EF56S)
    PRODUCT_PRIVATE_KEY := device/qcom/common/pantech_ef56s.key
else ifeq ($(TARGET_BUILD_PRJ_NAME),EF59S)
    PRODUCT_PRIVATE_KEY := device/qcom/common/pantech_ef59s.key
else ifeq ($(TARGET_BUILD_PRJ_NAME),EF59K)
    PRODUCT_PRIVATE_KEY := device/qcom/common/pantech_ef59k.key
else ifeq ($(TARGET_BUILD_PRJ_NAME),EF59L)
    PRODUCT_PRIVATE_KEY := device/qcom/common/pantech_ef59l.key
else ifeq ($(TARGET_BUILD_PRJ_NAME),EF60S)
    PRODUCT_PRIVATE_KEY := device/qcom/common/pantech_ef60s.key
else ifeq ($(TARGET_BUILD_PRJ_NAME),EF61K)
    PRODUCT_PRIVATE_KEY := device/qcom/common/pantech_ef61k.key
else ifeq ($(TARGET_BUILD_PRJ_NAME),EF62L)
    PRODUCT_PRIVATE_KEY := device/qcom/common/pantech_ef62l.key	
else ifeq ($(TARGET_BUILD_PRJ_NAME),EF63S)
    PRODUCT_PRIVATE_KEY := device/qcom/common/pantech_ef63.key	
else ifeq ($(TARGET_BUILD_PRJ_NAME),EF63K)
    PRODUCT_PRIVATE_KEY := device/qcom/common/pantech_ef63.key	
else ifeq ($(TARGET_BUILD_PRJ_NAME),EF63L)
    PRODUCT_PRIVATE_KEY := device/qcom/common/pantech_ef63.key	
else
...
ifneq ($(filter ef56% ef58% ef57% ef59% ef60% ef61% ef62% ef63%,$(TARGET_BUILD_PROJECT)),)
$(call inherit-product, build/target/product/nfc.mk)
endif
#+US1_CF2, US1_SYS_TESTING_TOOL
PRODUCT_PACKAGES += \
    testevent
#-US1_CF2
