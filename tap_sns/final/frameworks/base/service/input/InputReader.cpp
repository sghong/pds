...
void TouchInputMapper::dispatchTouches(nsecs_t when, uint32_t policyFlags) {
    ...
    if (currentIdBits == lastIdBits) {
        ...
        while (!downIdBits.isEmpty()) {
            ...
            // QEEXO START
            // Set touch kind information.
            setCurrentTouchKind(downId);
            // QEEXO END
...
// QEEXO START
void TouchInputMapper::setCurrentTouchKind(uint32_t downId) {
    // The pointer index for a specific finger may change across different motion events
    // so we need to find it via pointer id.
    const RawPointerData::Pointer& pointer = mCurrentRawPointerData.pointerForId(downId);
    struct raw_touch raw_touch = {0};
    raw_touch.x = pointer.x;
    raw_touch.y = pointer.y;
    raw_touch.touch_major = pointer.touchMajor;
    raw_touch.touch_minor = pointer.touchMinor;
    raw_touch.tool_major = pointer.toolMajor;
    raw_touch.tool_minor = pointer.toolMinor;
    raw_touch.pressure = pointer.pressure;
    raw_touch.orientation = pointer.orientation;
    raw_touch.id = pointer.id;
    // Get the touch kind associated with the last touch event.
    int touch_kind = fingersense_classify_touch(&raw_touch);
    ...
    // this information.
    uint32_t index = mCurrentCookedPointerData.idToIndex[downId];
    PointerCoords& out = mCurrentCookedPointerData.pointerCoords[index];
    out.setAxisValue(AMOTION_EVENT_AXIS_GAS, touch_kind);
}
// QEEXO END
...
void TouchInputMapper::cookPointerData() {
    ...
    for (uint32_t i = 0; i < currentPointerCount; i++) {
        ...
        // QEEXO START
        // Since we want to persist the touch kind for each pointer across touch events
        // (including the ACTION_MOVE event) we need to restore the previous value.
        if(mLastCookedPointerData.touchingIdBits.hasBit(in.id)) {
            uint32_t index = mLastCookedPointerData.idToIndex[in.id];
            PointerCoords& lastPointerCoords = mLastCookedPointerData.pointerCoords[index];
            float last_touch_kind = lastPointerCoords.getAxisValue(AMOTION_EVENT_AXIS_GAS);
            // NOTE: In order to avoid breaking the Android API, we are using the existing AMOTION_EVENT_AXIS_GAS
            // constant as a temporary axis value to provide the touch kind to Java applications.
            // In the future, once we know we can modify the API, we can use the tool type to represent
            // this information.
            out.setAxisValue(AMOTION_EVENT_AXIS_GAS, last_touch_kind);
        }
        // QEEXO END
